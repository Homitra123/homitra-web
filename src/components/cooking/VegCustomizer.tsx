...
import { VEG_OPTIONS, getVegPrice, getRotiQty } from '../../data/cookingPlans';

export interface VegState {
  people: number;
  vegetable: string;
...
const VegCustomizer = ({ onChange }: VegCustomizerProps) => {
  ...
  const addOns = (state.extraRotisCount / 5) * 25 + (state.addExtraVeg ? 70 : 0);
  const totalPrice = basePrice + addOns;
  const isValid = !!state.vegetable;

  const availableExtraVeg = state.vegetable === 'Any green veg'
    ? VEG_OPTIONS.vegetables
    : VEG_OPTIONS.vegetables.filter(v => v !== state.vegetable);

  useEffect(() => {
    onChange(state, totalPrice, isValid);
  }, [state]);

  const update = (patch: Partial<VegState>) => setState(prev => ({ ...prev, ...patch }));
        ...
        <div className="space-y-3">
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-lg px-4 py-3">
            <div className="flex items-center justify-between mb-2">
              <div>
                <div className="text-sm font-medium text-gray-800">Extra Vegetable</div>
                <div className="text-xs text-gray-500">
                  {state.vegetable === 'Any green veg'
                    ? 'You can add another veg dish (including another green veg)'
                    : 'Add a second veg dish'}
                </div>
              </div>
              <span className="text-sm font-bold text-orange-600">+ ₹70</span>
            </div>
            <select
              value={state.addExtraVeg}
              onChange={e => update({ addExtraVeg: e.target.value })}
              disabled={!state.vegetable}
            ...
            >
              <option value="">
                {state.vegetable ? 'Select extra vegetable (optional)' : 'Select a vegetable first'}
              </option>
              {availableExtraVeg.map(v => (
                <option key={v} value={v}>{v}</option>
              ))}
            </select>
          </div>
